@extends('admin.main')
@push('title')
<title>Doctors || Medical Report Meker</title>
@endpush
@section('content')
<div class="sent_reports border p-4 ">
    ...
    <div class="row sraction mb-4">
        <form action="{{url('/admin/doctors/')}}" class="row sraction">
        ...
        <div class="col-lg-4 col-md-4 text-end">
           <a name="" id="" class="btn btn-success text-end" href="{{url('/admin/add-doctor')}}" role="button">+ Add New</a>
        </div>
        </form>
    </div>
    <div class="row">
        @if (!app('request')->input('search') == "" || !app('request')->input('type') == "" || !app('request')->input('date') == "")
        <div class="col-12 text-center mb-2"><a href="{{url('/admin/doctors')}}" class="btn btn-danger">Cancel Filter</a></div>                    
        @endif
    </div>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="m-0 d-flex justify-content-around border-0">
                        <a href="{{url('/admin/doctor-profile/'.$item['did'])}}"><span role="button"><img src="{{url('/assets/image/view.png')}}" alt="view"></span></a>
                        <a href="{{url('/admin/doctor-action/'.$item['did'])}}"><span role="button"><img src="{{url('/assets/image/edit.png')}}" alt="Edit"></span></a>
                    </td>
                </tr>
                @endforeach
                @else
